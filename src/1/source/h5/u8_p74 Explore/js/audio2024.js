		...
		for(var i=0;i<timeArr.length;i++){
			...
			if(i%2==0){//每一句开始时间,要显示字幕（如果设置了关闭字幕，则不显示，如果设置中英文切换，则对应显示）
				if(millTime>=t1&&millTime<=t2){                				
					...
					if (isEnglish) {
					    var currentText = wordArr[wordIndex];
					
					    // 判断文本是否为 "I like Monkey King."
					    if (currentText === "I like Monkey King.") {
					        // 替换 "Monkey King" 为斜体
					        currentText = currentText.replace("Monkey King", "<i class='danyh'>Monkey King</i>");
					    } else {
					        // 对包含 "'" 的文本做斜体处理
					        if ((currentText.indexOf("'") != -1) && (currentText.indexOf("</i>") < 0)) {
					            currentText = currentText.replace(/'/ig, "<i class='danyh'>'</i>");
					        }
					    }
					
					    // 更新 wordbox 内容
					    $(wordbox).html("<span>" + currentText + "</span>");
					} else {
